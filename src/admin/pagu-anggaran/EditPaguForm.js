  ...
  FormInputs(){
    ...
    ado_list.forEach(ado =>{
        pagu_list.forEach(pagu =>{
          if((pagu.unit === this.props.selectedUnit.unit) && (pagu.subunit === this.props.selectedUnit.sub_unit) && (pagu.ADO === ado)){
            value = pagu.alokasi;
          }
        });

        inputs.push(
          <Form.Group controlId={this.props.selectedUnit.unit + " " + this.props.selectedUnit.sub_unit + " " + ado}>
            <Form.Label>{ado}</Form.Label>
            <Form.Control type="number" required value={value} onChange={this.props.onChange}
            />
          </Form.Group>
        );
      });
        ...
        <Modal show onHide={() => this.props.hide()}>
          ...
          <Modal.Body>
            ...
            <Form.Group controlId="formPlaintextUnit">
              <Form.Label column sm="2">{this.props.selectedUnit.unit}</Form.Label>
            </Form.Group>
            <Form.Group controlId="formPlaintextSubUnit">
              <Form.Label column sm="2">{this.props.selectedUnit.sub_unit}</Form.Label>
            </Form.Group>
              {this.FormInputs()}
