...
export default class ModalTambahAlokasi extends Component {
  ...
  handleSelectBulan = ({ target: { value } }) =>
    this.props.updateSelectedBulan(value);
  handleSubmitForm = (e) => {
    e.preventDefault();
    ...
    return (
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Tambah Alokasi Bulan {bulan}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <Form onSubmit={this.handleSubmitForm}>
            <Form.Group controlId="pilih-bulan">
              ...
              <Form.Control as="select" onChange={this.handleSelectBulan}>
                {namaBulanIndonesia.map((b, id) => {
                  ...
                  return (
                    <option key={id} selected={b === selectedBulan}>
                      {b}
                    </option>
                  );
                })}
              ...
              {!!selectedBulan && (
                <Table responsive="sm" bordered>
                  <tbody>
                    <tr>
                      <td>
                        <b>Sisa Anggaran Bulan {selectedBulan}</b>
                      </td>
                      <td>
                        {formatRupiah(
                          getSisaAnggaranFromBulan(RKA, selectedBulan)
                        )}
                      </td>
                    </tr>
                  </tbody>
            ...
            <Form.Group controlId="tambah-alokasi-dari-bulan">
              <Form.Label>
                Masukkan jumlah alokasi yang ingin diambil dari bulan{" "}
                <b>{selectedBulan}</b>
              </Form.Label>
              <Form.Control type="number" ref={this.amount} />
              <Form.Text id="tambah-alokasi-feedback" muted>
                Pastikan jumlah alokasi tidak melebihi sisa anggaran bulan{" "}
                {selectedBulan}
              </Form.Text>
            </Form.Group>
