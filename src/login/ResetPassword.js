...
export class ResetPassword extends Component {
  ...
  render() {
    ...
    if (!user) {
      return (
        <Alert variant="danger" className="p-3 text-center">
          Error 404 Not Found
        </Alert>
      );
    }
    ...
    return (
      <Container className="d-flex justify-content-center align-items-center">
        <Card className="mt-5 text-center" style={{ width: "45%" }}>
          <Card.Header as="h5">
            Reset Kata Sandi User: <b>{user.username}</b>
          </Card.Header>
          <Card.Body>
            <Formik
                ...
                <Form noValidate onSubmit={handleSubmit}>
                  <Form.Group controlId="password-form">
                    <Form.Label className="d-flex justify-content-center align-items-center">
                      <RiLockPasswordLine className="mr-1" /> Kata Sandi Baru
                    </Form.Label>
                    <Form.Control
                      required
                      type="password"
                  ...
                  <Form.Group controlId="password-confirm-form">
                    <Form.Label className="d-flex justify-content-center align-items-center">
                      <RiLockPasswordFill className="mr-1" /> Konfirmasi Kata
                      Sandi
                    </Form.Label>
                    <Form.Control
                      required
                  ...
                  <Button
                    type="submit"
                    variant="info"
                    disabled={!!feedback && feedback.status === "success"}
                  >
                    Submit
                  </Button>
                  {feedback !== null && (
                    <Alert className="mt-3" variant={feedback.status}>
                      {feedback.message}
                    </Alert>
                  )}
                </Form>
          ...
          <Card.Footer>
            {!!feedback && (
              <Link to="/login" exact className="text-black-50">
